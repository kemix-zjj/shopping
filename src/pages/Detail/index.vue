...
export default {
  ...
  methods: {
    //产品的售卖属性值切换高亮  选择即高亮，其余不高亮 -- 排他思想
    // 因为有用到排他思想，不仅要获得点击的那个属性，其的兄弟属性也会操作到，因此也要获取其他兄弟的属性
    // 即最后所要传递的参数是 点击的属性  与  存放该属性的数组
    changeActive(saleAttrValue, arr) {
      // 排他思想
      // 遍历全部的售卖属性值isChecked为0 没有高亮了
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      // 点击的那个售卖属性值为高亮
    ...
    async addShopCar() {
      /**
       * 当前这里派发的一个action.也想向服务器发请求，判断加入购物车是成功还是失败了，再进行相应的操作。
       * this.$store.dispatch('addOrUpdateShopCart', {skuId: this.$route.params.skuid,skuNum: this.skuNum,});
       * 上面这行代码说白了：调用仓库中的addOrUpdateShopCart,这个方法加上了async,返回的一定是一个Promise
       * promise返回的要么是成功，要么是失败
       */

      // 1.在点击加入购物车这个按钮的时候，做的第一件事，将参数带给服务器（发请求），通知服务器加入购物车的产品是谁
      // this.$store.dispatch('addOrUpdateShopCart')说白了，它是在调用vuex仓库中的这个addOrUpdateShopCart函数
      // 2.你需要做的这次请求成功还是失败，如果成功进行路由跳转，如果失败，需要给用户提示
      try {
        // 成功
        ...
        //3.进行路由跳转
        // 4.在路由跳转的时候，还需要将产品的信息带给下一级的路由组件
        // 一些简单的数据skuNum，通过query形式给路由组件传递过去
        // 产品信息的数据【比较复杂：skuInfo】,通过会话存储进行传递（不持久化，会话结束数据消失）
        // 本地存储和会话存储，一般存储的是字符串
